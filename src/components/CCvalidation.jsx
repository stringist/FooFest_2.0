    ...
    <>
      ...
      <form ref={formEl} onChange={changeFocus} onSubmit={handleSubmit} className={Creditcard.cc_form}>
        ...
        <section className={Creditcard.section_cc}>
          <span>Expiry</span>
          <input type="text" value={handleExpiryDate()} onChange={(e) => setExpiry(e.target.value)} onFocus={(e) => setFocus(e.target.name)} onInput={(e) => handleExpiryDate(e)} minLength="5" maxLength="5" required />
          <span>CVC</span>
          {number.substring(0, 2) == 34 || number.substring(0, 2) == 37 ? (
            ...
            <input type="tel" name="cvc" value={cvc} onChange={(e) => setCvc(e.target.value)} onFocus={(e) => setFocus(e.target.name)} minLength="3" maxLength="3" required />
          )}
        </section>
        <section className={generalStyles.next_back_buttons}>
          <button onClick={changePage}>Back</button>
          <button onSubmit={(e) => handleSubmit()} onFocus={(e) => setFocus(e.target.name)}>
            Complete purchase
          </button>
        </section>
      </form>
